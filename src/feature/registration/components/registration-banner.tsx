...
const highlights = [
  ...
  'Free material persiapan newborn',
];
...
export function RegistrationBanner() {
  return (
    <section className='relative overflow-hidden bg-linear-to-br from-pink-400 via-pink-700 to-pink-300 text-white'>
        ...
        <div className='flex flex-col items-center gap-4 text-center md:gap-6'>
          ...
          <div className='flex flex-col gap-3 text-pretty'>
            <div>
              ...
              <p className='text-sm font-medium text-white sm:text-base'>by Dr. Beryliana Maya Anisa, Sp.OG, CLC</p>
            </div>
          ...
          <div className='grid w-full gap-4 rounded-2xl bg-white/10 p-4 backdrop-blur sm:grid-cols-2'>
            {highlights.map((item) => (
              <div key={item} className='flex items-start gap-3 text-left md:items-center'>
          ...
          <div className='flex flex-wrap items-center justify-center gap-3 text-sm text-white/70'>
            {details.map((detail) => (
              <div key={detail.label} className='rounded-full border border-white/30 px-4 py-1 backdrop-blur'>
                <span className='text-[11px] font-bold tracking-wide uppercase'>{detail.label}</span>
